   ...
   /*private HashMap<String, Member> memberdb;*/
    ...
    public Member select(Member member) {
        ...
        try{
            ...
            conn.prepareStatement("SELECT * FROM Member WHERE ?");
            ...
            if(resultSet.next()){ // 찾으려는 아이디 값이 있을 경우
                ...
                while(resultSet.next()){
                    ...
                    selectMember.setMemId(resultSet.getString(2));
                    selectMember.setMemId(resultSet.getString(3));
                }
            }
            return selectMember;
        }catch (Exception e){
    ...
    public int update(Member member) {
        try{
            ...
            conn.prepareStatement("UPDATE Member SET memId = ?, memPw = ?, memMail = ? WHERE memId = ?");
            pstmt.setString(1, member.getMemId());
            pstmt.setString(2, member.getMemPw());
            pstmt.setString(3, member.getMemMail());
    ...
    public int delete(Member member) {
        try{
            ...
            conn.prepareStatement("DELETE FROM Member WHERE memId = ?");
            pstmt.setString(1, member.getMemId());
            return pstmt.executeUpdate();
        }catch (Exception e){
            e.printStackTrace();
